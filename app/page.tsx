...
function StoryModal({ story, lang, onClose }: { story: Story; lang: Lang; onClose: () => void }) {
  ...
  return <div className="story-modal" role="dialog" aria-modal="true" aria-label={story.title} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <article>
      ...
      {story.placeholder ? <div className="confidential-art"><span>CONFIDENTIAL</span><b>Product × Flow</b><i>Details intentionally withheld</i></div> : <img className={story.imageClass} src={story.image} alt={story.alt} />}
...
function StoryCard({ story, index, lang, onOpen }: { story: Story; index: number; lang: Lang; onOpen: () => void }) {
  return <button className="story-card" data-reveal onClick={onOpen} aria-label={`${copy[lang].open}: ${story.title}`}>
    <div className={`story-image ${story.imageClass ?? ""}`}>{story.placeholder ? <div className="confidential-art"><span>CONFIDENTIAL</span><b>Product × Flow</b><i>Identity protected</i></div> : <img src={story.image} alt="" />}<span>0{index + 1}</span></div>
    ...
    <section className="hero" id="home">
      ...
      <div className="hero-grid">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><a className="button primary" href="#research">{t.researchCta} <span>↓</span></a><a className="button secondary" href="/Xingtong-Lin-Resume.pdf" target="_blank" rel="noreferrer">{t.resume} <span>↗</span></a></div>
        </div>
        <aside className="hero-visual" aria-label={lang === "en" ? "Portrait of Xingtong Lin" : "林星潼个人照片"}>
          <div className="hero-profile">
            <div className="profile-photo"><div className="portrait-label"><span>CMU</span><b>NEUROSCIENCE · NEUROENGINEERING</b></div><img src="/xingtong-profile.jpg" alt={`Portrait of ${t.name}`} /></div>
    ...
    <section className="contact section-pad" id="contact" data-reveal>
      <div className="contact-shell">
        ...
        <div className="social-icons">
          <a href="https://www.linkedin.com/in/lxt/" target="_blank" rel="noreferrer" aria-label="LinkedIn"><b className="app-logo"><img src="/social/linkedin.svg" alt="" /></b></a>
          <a href="https://xhslink.cn/m/6kHp1Z9VdG5" target="_blank" rel="noreferrer" aria-label="小红书"><b className="app-logo"><img src="/social/xiaohongshu.svg" alt="" /></b></a>
          <button type="button" onClick={copyWeChat} aria-label={lang === "en" ? "Copy WeChat ID" : "复制微信号"}><b className="app-logo"><img src="/social/wechat.svg" alt="" /></b></button>
        </div>
      </div>
